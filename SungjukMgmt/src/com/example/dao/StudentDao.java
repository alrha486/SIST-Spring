...
public class StudentDao {
	...
	public int create(Student s) {
		String sql = "INSERT INTO Student(hakbun, name, kor, eng, mat, edp, " +
	                      "sum, avg, grade) VALUES('" + s.getHakbun() + "', '" + s.getName() +
	                      "', " + s.getKor() + ", " + s.getEng() + ", " + s.getMat() + 
	                      ", " + s.getEdp() + ", " + s.getSum() + ", " + s.getAvg() +
	                      ", '" + s.getGrade() + "')";
	...
	private class MyRowMapper implements RowMapper<Student> {
		...
		public Student mapRow(ResultSet rs, int rowNum) throws SQLException {
			...
			s.setHakbun(rs.getString("hakbun"));  s.setName(rs.getString("name"));
			s.setKor(rs.getInt("kor"));   s.setEng(rs.getInt("eng"));
			s.setMat(rs.getInt("mat"));  s.setEdp(rs.getInt("edp"));
			s.setSum(rs.getInt("sum"));  s.setAvg(rs.getDouble("avg"));
